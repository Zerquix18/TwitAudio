...
class MobileAJAXController
{
    ...
    public function __construct($via, $method, $action)
    {
        ...
        **/
        ...
        if (in_array($action, array('__construct', 'setRules'))) {
            View::exit404();
        }
        /**
    ...
    **/
    public function setRules(array $options)
    {
        ...
        if (! is_logged()) {
            /**
             * This is only for the web
             * In the mobile side, \Sessions::init will exit
             */
            HTTP::exitJson(array(
                    'success'  => false,
    ...
    **/
    private function audios()
    {
        ...
        $page = HTTP::get('p');
        $page = HTTP::sanitizePageNumber($page) ?: 1;
        ...
        $result  = Audios::getAudios($user['id'], $page);
        // Mobile side:
        if ('mob' == $this->via) {
            HTTP::exitJson(array('success' => true) + $result);
        }
        // AJAX side:
        $response = '';
        foreach ($result['audios'] as $audio) {
            $response .= View::getPartial('audio', $audio);
        }
    ...
    **/
    private function audio()
    {
        ...
        HTTP::exitJson(array('success' => true) + $audio);
    }
    /**
    ...
    **/
    private function checkeffects()
    {
        ...
        $loaded_effects = Audio::getFinishedEffects($_SESSION[$id]['effects']);
        $loaded_effects_count = count($loaded_effects);

        $current_user = Users::getCurrentUser();

        $total          = count($current_user->getAvailableEffects());
    ...
    **/
    private function cut()
    {
        ...
        if ($difference > $current_user->getLimit('audio_duration')) {
            throw new \ValidationException(
        ...
        $available_effects         = $current_user->getAvailableEffects();

        $_SESSION[$id]['effects']  = Audio::applyEffects(
                                        $audio->audio,
                                        $available_effects
                                );

        $total_effects = Audio::getEffects();
        $effects = array();
        foreach ($total_effects as $effect_name => $effect_name_public) {
    ...
    **/
    private function favorite()
    {
        ...
        if (! $audio['is_favorited'] && 'fav' == $action) {
            Audios::registerFavorite($audio['id']);
            $count += 1;
        }elseif ($audio['is_favorited'] && 'unfav' == $action) {
            Audios::unregisterFavorite($audio['id']);
            $count -= 1;
        }
    ...
    **/
    private function favorites()
    {
        ...
        $result  = Audios::getFavorites($user['id'], $page);
        // Mobile side:
        if ('mob' == $this->via) {
            HTTP::exitJson(array('success' => true) + $result);
        }
    ...
    private function home()
    {
        ...
        $data   = array(
                'recent_popular' => array(
                        'audios' => Audios::getPopularAudios()
                ),
                'recent_audios'  => array(
                        'audios' => Audios::getRecentAudios()
                )
            );
        HTTP::exitJson(array('success' => true) + $data);
    }
    /**
    ...
    **/
    private function play()
    {
        ...
        $count         = $audio['plays'];
        $register_play = Audios::registerPlay($id);
    ...
    **/
    private function profile()
    {
        ...
        HTTP::exitJson(array('success' => true) + $user_info);
    }
    /**
    ...
    **/ 
    private function replies()
    {
        ...
        $replies = Audios::getReplies($audio_id, $page);
        /** LINKED REPLIES **/
        // here be dragons...
        if (   'ajax' == $this->via
        ...
        // Mobile side:
        if ('mob' == $this->via) {
            HTTP::exitJson(array('success' => true) + $replies);
        }
        // AJAX side:
        $response = '';
        foreach ($replies['audios'] as $audio) {
            $response .= View::getPartial('audio', $audio);
        }
        $response = minify_html($response);
    ...
    **/
    private function reply()
    {
        ...
        // Mobile SIDE:
        if ('mob' == $this->via) {
            HTTP::exitJson(array('success' => true) + $reply);
        }
        // AJAX side:
        $response = View::getPartial('audio', $reply);
        $response = minify_html($response);
        HTTP::exitJson(array(
                'success'  => true,
                'response' => $response
    ...
    **/
    private function search()
    {
        ...
        foreach ($result['audios'] as $audio) {
            $response .= View::getPartial($partial, $audio);
        }
        $response = minify_html($response);
    ...
    **/
    private function upload()
    {
        ...
        $current_user = Users::getCurrentUser();
        $file_limit   = $current_user->getLimit('file_upload');
        ...
        **/
        ...
        $audio = new \application\Audio($file, array(
            'validate'         => true,
            'max_duration'     => $current_user->getLimit('audio_duration'),
            'is_voice'         => $is_voice,
            'decrease_bitrate' => ! $current_user->isPremium(),
            )
        );
